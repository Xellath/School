/*
 * guess_nr.c
 * Guess_nr handles the core of the game. Takes a number from the player and checks it from the randomized number. Shows the result on the screen. 
 *
...
static uint16_t get_nr(void)
{
	...
	while (1) {
		input_length = input_int("ENTER NUMBER:", &guessed_nr);//UPPGIFT 8.2.2 Sends the guessed nubmer to input_int and also the text string "Enter number".
...
static uint8_t playing_game(uint16_t rnd_nr, uint16_t *p_nr_of_guesses)
{
	...
	//UPPGIFT 8.2.3 Increses the guesses count.
	(*p_nr_of_guesses)++;
