...
uint8_t input_int(char *p_msg, uint16_t *p_int_nr)
{
	...
	while (1) {
		...
		if (key == '#') {					// confirmation of the number?
			...
			if (length > 0) {
				//UPPGIFT 7.2.2 - Erases the last digit on row 1. 
			...
			if(key == 'A' || key == 'B' || key == 'C' || key == 'D') { // Uppgift 7.2.4 - Disables A,B,C,D 
		...
		//UPPGIFT 7.2.1 
		// continue loop when key is released!
		while(numkey_read() != NO_KEY) {
			//Waits untill key is released.
		}
	};
	// terminate string
	numbers[length] = '\0';
